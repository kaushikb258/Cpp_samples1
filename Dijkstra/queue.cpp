...
void queue::chgPrioirity(int node, double v, int prev_node)
{
 if(this->contains(node))
 { 
...
double queue::get_value(int node)
{
 double val = -1.0;
 if (this->contains(node))
 {
...
void queue::minPriority(int &top_node, double &top_node_value, int &top_node_prev)
{
 top_node = -1;
 top_node_value = 9999999999.99; 
 top_node_prev = -1;
 int ii = -1;
...
void queue::Insert(int node, double v, int prev_node)
{
